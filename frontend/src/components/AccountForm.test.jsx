...
describe('AccountForm', () => {
  ...
  it('renders "New Account" title when no account prop', () => {
    renderWithProviders(<AccountForm onSave={onSave} onClose={onClose} />)
    expect(screen.getByText('New Account')).toBeInTheDocument()
  })

  it('renders "Edit Account" title when account prop is provided', () => {
    renderWithProviders(<AccountForm account={{ id: '1', name: 'Checking', account_type: 'CHECKING', currency: 'USD', initial_balance: 0 }} onSave={onSave} onClose={onClose} />)
    expect(screen.getByText('Edit Account')).toBeInTheDocument()
  })

  it('pre-fills name when editing', () => {
    renderWithProviders(<AccountForm account={{ id: '1', name: 'My Account', account_type: 'SAVINGS', currency: 'EUR', initial_balance: 500 }} onSave={onSave} onClose={onClose} />)
    expect(screen.getByDisplayValue('My Account')).toBeInTheDocument()
  })

  it('pre-fills account type when editing', () => {
    renderWithProviders(<AccountForm account={{ id: '1', name: 'X', account_type: 'SAVINGS', currency: 'USD', initial_balance: 0 }} onSave={onSave} onClose={onClose} />)
